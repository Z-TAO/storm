...
(defn read-local-worker-executors [assignment-id port executor-id->tasks]
  (log-message "Reading Assignments.")
  (doall
    (concat
      [Constants/SYSTEM_EXECUTOR_ID]
...
(defn worker-data [conf mq-context storm-id assignment-id port worker-id topology storm-conf executor->node+port]
  (let [assignment-versions (atom {})
        ...
        executors (set (read-local-worker-executors assignment-id port executor->node+port))
...
(defserverfn mk-worker [conf shared-mq-context storm-id assignment-id port worker-id topology storm-conf executor->node+port]
